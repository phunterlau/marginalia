...
def test_native_command_registration_is_offline_and_minimal(setup):
    ...
    async def run():
        ...
        assert {cmd.name for cmd in client.tree.get_commands()} == {"new", "resume", "fork", "session", "space", "ask", "stop", "paper", "card", "publish", "discussed", "recall", "compare", "brainstorm", "save"}
